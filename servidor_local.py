...
CABECALHO_OUT = ['CPF Buscado', 'Nome do Cliente', 'Contrato', 'Código do Usuário',
                 'Plano', 'Início', 'Cancelamento', 'Motivo do Cancelamento']
CABECALHO_REP = CABECALHO_OUT + ['Plano (Repique)']


def _tentar_com_retry(fn, tentativas: int = 8, espera: float = 0.5) -> bool:
    """Roda fn() várias vezes antes de desistir — usado para lidar com
    arquivos temporariamente bloqueados no Windows (ex.: CSV aberto no
    Excel). Retorna True se deu certo, False se continuou bloqueado."""
...
def resetar_arquivos_saida() -> List[str]:
    """Recria resultados_beneficios.csv e Repiques_Ativos.csv vazios (só
    cabeçalho), ANTES de iniciar o robô. Se algum estiver aberto no Excel,
    detectamos aqui e avisamos o usuário na hora — em vez de deixar o
    robô começar a "rodar" sem nunca ter conseguido limpar o arquivo
    antigo (o que fazia o download parecer trazer dados da 1ª consulta)."""
    bloqueados = []
    for arq, cabecalho in ((ARQUIVO_OUT, CABECALHO_OUT), (ARQUIVO_REP, CABECALHO_REP)):
        def escrever(arq=arq, cabecalho=cabecalho):
            with open(arq, "w", newline="", encoding="utf-8-sig") as f:
                csv.writer(f, delimiter=";").writerow(cabecalho)
        if not _tentar_com_retry(escrever):
            bloqueados.append(arq.name)
    return bloqueados
...
def iniciar_robo(cfg: ConfigRobo):
    ...
    # Limpa/recomeça os CSVs de saída ANTES de abrir o robô. Se algum
    # estiver aberto no Excel, avisamos aqui e BLOQUEAMOS o início —
    # assim o usuário sabe na hora que precisa fechar o arquivo, em vez
    # de o robô "iniciar" e nunca ter limpado os dados da consulta anterior.
    bloqueados = resetar_arquivos_saida()
    if bloqueados:
        raise HTTPException(
            423,
            f"Feche estes arquivos antes de iniciar (estão abertos no Excel ou outro programa): "
            f"{', '.join(bloqueados)}"
        )

    # Apaga os resultados da análise anterior ANTES de abrir o robô — segunda
    # garantia além do reset que o próprio roboconsulta_paralelo.py faz.
    # Isso cobre o caso de "Limpar Tudo" ter deixado algo pra trás e,
    # principalmente, detecta se o CSV está TRAVADO (ex: aberto no Excel).
    # Se estiver travado, o open("w") dentro do robô falharia em silêncio
    # (o processo filho morre sem gerar nada novo) e o download continuaria
    # servindo o arquivo antigo — por isso aqui a gente falha alto, com uma
    # mensagem clara, em vez de deixar isso acontecer.
    for arq in (ARQUIVO_OUT, ARQUIVO_REP):
        try:
            if arq.exists():
                arq.unlink()
        except PermissionError:
            raise HTTPException(
                409,
                f"Não foi possível resetar '{arq.name}' — ele está aberto em "
                f"outro programa (ex: Excel). Feche o arquivo e tente novamente."
            )
        except Exception as e:
            raise HTTPException(500, f"Erro ao resetar '{arq.name}': {e}")
...
def limpar_dados():
    ...
    for arq in (ARQUIVO_CPF, ARQUIVO_OUT, ARQUIVO_REP):
        if not arq.exists():
            continue
        if _tentar_com_retry(arq.unlink):
            removidos.append(arq.name)
        else:
            erros.append(f"{arq.name}: arquivo aberto no Excel (ou outro programa) — feche-o e tente de novo")
